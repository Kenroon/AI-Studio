...
// Logos centrales de alto impacto: Negro sólido, formas masivas
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    if (state.bodyWeightLogs.length === 0) {
  ...
  return (
    <Layout activeTab={activeTab} setActiveTab={setActiveTab} settings={state.settings}>
      <div className="pb-10 min-h-[80vh]">
        {activeTab === 'routines' && (
          <div className="animate-in space-y-6 relative h-full">
            {/* Logo CENTRAL de gran impacto */}
            <div 
              className={`fixed inset-0 pointer-events-none flex items-center justify-center ${isDark ? 'opacity-[0.08]' : 'opacity-[0.04]'}`}
            >
              <div 
                className="w-full h-full max-w-[300px] max-h-[300px] bg-center bg-no-repeat bg-contain" 
                style={{ backgroundImage: currentPattern }}
              ></div>
            </div>
            ...
            <header className="flex justify-between items-end relative z-10">
              ...
              <button 
                onClick={() => setEditingRoutine({ id: Date.now().toString(), name: '', exerciseIds: [] })} 
                className="bg-white dark:bg-gray-800 p-3 rounded-full shadow-md active:scale-90 transition-transform"
                style={{ color: accent }}
              >
            ...
            <div className="space-y-4 relative z-10">
              {state.routines.map(r => (
                <div 
                  key={r.id} 
                  className={`p-6 rounded-[2.5rem] shadow-sm flex justify-between items-center group cursor-pointer border-2 transition-all active:scale-[0.98] ${isDark ? 'bg-gray-800/80 backdrop-blur border-transparent hover:border-gray-600' : 'bg-white/80 backdrop-blur border-transparent hover:border-gray-100'}`} 
                  onClick={() => setViewingRoutine(r)}
                >
                  <div>
                    <h3 className="text-xl font-bold">{r.name}</h3>
                    <p className="text-xs font-black text-gray-400 uppercase tracking-wider">{r.exerciseIds.length} Ejercicios</p>
                  </div>
        ...
        {activeTab === 'exercises' && (
          <div className="animate-in space-y-8">
            <header className="flex justify-between items-end">
              <div>
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Biblioteca Personal</p>
            ...
            <div className="space-y-10">
              {MUSCLE_GROUPS.map(mg => (
                <div key={mg}>
                  <h2 className="text-xl font-bold mb-4 ml-4">{mg}</h2>
                  <div className={`rounded-[2.5rem] overflow-hidden shadow-sm border ${isDark ? 'bg-gray-800/30 border-gray-700' : 'bg-white border-gray-50'}`}>
                    {state.customExercises.filter(e => e.muscleGroup === mg).map((e, idx, arr) => (
                      <div key={e.id} onClick={() => { if(!isLibraryEditMode) { setSelectedExerciseId(e.id); setExerciseDetailTab('entrenar'); } }} className={`p-5 flex justify-between items-center transition-colors ${!isLibraryEditMode && 'cursor-pointer hover:bg-gray-100/10'} ${idx !== arr.length - 1 ? (isDark ? 'border-b border-gray-700' : 'border-b border-gray-50') : ''}`}>
            ...
            <section className="space-y-4">
              ...
              <div className={`p-6 rounded-[2.5rem] shadow-sm border space-y-6 ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-50'}`}>
                <div className="flex justify-between items-center">
                  ...
                  <button 
                    onClick={() => handleUpdateSetting('theme', isDark ? 'light' : 'dark')}
                    className={`w-14 h-8 rounded-full transition-colors relative ${isDark ? 'bg-apple-green' : 'bg-gray-300'}`}
                  >
                    <div className={`absolute top-1 w-6 h-6 bg-white rounded-full transition-all ${isDark ? 'left-7' : 'left-1'}`}></div>
                  </button>
                </div>
                
                <div className="space-y-3">
                  <span className="text-xs font-bold text-gray-400">Color de Acento</span>
                  <div className="grid grid-cols-4 gap-3">
                    {ACCENT_COLORS.map(c => (
                      <button 
                        key={c.name} 
                        onClick={() => handleUpdateSetting('accentColor', c.color)}
                        className={`h-12 rounded-2xl border-2 transition-all ${state.settings.accentColor === c.color ? 'border-gray-400 scale-105 shadow-md' : 'border-transparent opacity-60'}`}
                ...
                <div className="space-y-3">
                  <span className="text-xs font-bold text-gray-400">Fondo Central</span>
                  <div className="grid grid-cols-2 gap-2">
                    {PATTERNS.map(p => (
                      <button 
                        key={p.id} 
                        onClick={() => handleUpdateSetting('bgPattern', p.id)}
                        className={`py-3 rounded-2xl text-[10px] font-black uppercase border-2 transition-all overflow-hidden relative ${state.settings.bgPattern === p.id ? 'border-gray-400 bg-gray-100 dark:bg-gray-700' : 'border-transparent bg-gray-50 dark:bg-gray-900 opacity-60'}`}
                      >
                        <div className="absolute inset-0 opacity-10 pointer-events-none scale-50 bg-center bg-no-repeat bg-contain" style={{ backgroundImage: p.id === 'none' ? '' : (isDark ? p.svg.replace(/black/g, 'white') : p.svg) }}></div>
      ...
      {/* Detalle Rutina */}
      {viewingRoutine && (
        <div className={`fixed inset-0 z-[60] p-6 overflow-y-auto animate-in ${isDark ? 'bg-[#1c1c1e]' : 'bg-[#f2f2f7]'}`}>
          <div className="flex justify-between items-center mb-10"><button onClick={() => setViewingRoutine(null)} className={`font-bold w-10 h-10 rounded-full flex items-center justify-center shadow-sm ${isDark ? 'bg-gray-800' : 'bg-white'}`}><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M15 19l-7-7 7-7" /></svg></button><h2 className="text-lg font-black uppercase text-gray-300 truncate px-4">{viewingRoutine.name}</h2><div className="w-10"></div></div>
      ...
      {/* Detalle Ejercicio Modal */}
      {selectedExerciseId && (
        <div className={`fixed inset-0 z-[80] p-6 overflow-y-auto animate-in flex flex-col ${isDark ? 'bg-[#1c1c1e]' : 'bg-white'}`}>
          <div className="flex justify-between items-center mb-6"><button onClick={() => setSelectedExerciseId(null)} className={`font-bold w-10 h-10 rounded-full flex items-center justify-center ${isDark ? 'bg-gray-800' : 'bg-gray-100'}`}><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" /></svg></button><h2 className="text-lg font-black uppercase tracking-widest text-gray-300">Detalles</h2><div className="w-10"></div></div>
          ...
          <div className="flex-1">
            ...
            {exerciseDetailTab === 'historial' && (
              <div className="space-y-6 pb-20">
                {exerciseHistory(selectedExerciseId).slice().reverse().map((h, i) => (
                  <div key={i} className={`p-5 rounded-[2rem] ${isDark ? 'bg-gray-800' : 'bg-[#f2f2f7]'}`}><div className="flex justify-between mb-3 items-center"><span className="text-[10px] font-black text-gray-400 uppercase">{h.date}</span></div>
                    <div className="space-y-2">{h.log?.sets.map((s, idx) => (
                        <div key={idx} className={`p-3 rounded-xl ${isDark ? 'bg-gray-700/50' : 'bg-white/60'}`}><div className="flex justify-between text-sm font-bold opacity-80"><span>Serie {idx+1}</span><span>{s.weight}kg x {s.reps}</span></div>{s.note && <p className="text-[10px] text-gray-400 mt-1 italic">"{s.note}"</p>}</div>
                    ))}</div>
                  </div>
                ))}
              </div>
            )}
            {exerciseDetailTab === 'progreso' && (
              <div className="space-y-6 pb-20"><section className={`p-6 rounded-[3rem] shadow-sm border ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-cream border-[#f5f1d7]'}`}><SimpleChart data={getVolumeData(selectedExerciseId)} color={accent} /></section></div>
            )}
          </div>
        </div>
      )}
      ...
      {/* Modal Editar Rutina */}
      {editingRoutine && (
        <div className={`fixed inset-0 z-[70] p-6 overflow-y-auto animate-in ${isDark ? 'bg-[#1c1c1e]' : 'bg-white'}`}>
          <div className="flex justify-between items-center mb-10"><button onClick={() => setEditingRoutine(null)} className="text-gray-400 font-bold">Cancelar</button><button onClick={() => handleSaveRoutine(editingRoutine)} className="font-black bg-green-50 dark:bg-green-900/20 px-5 py-2 rounded-full" style={{ color: accent }}>Guardar</button></div>
          ...
          <div className="space-y-10 pb-20">{MUSCLE_GROUPS.map(mg => (
            <div key={mg}><h4 className="text-[10px] font-black text-gray-400 uppercase mb-4 tracking-[0.2em]">{mg}</h4><div className="grid grid-cols-1 gap-2">
              {state.customExercises.filter(e => e.muscleGroup === mg).map(e => (
        ...
        <div className="fixed inset-0 z-[100] apple-blur flex items-center justify-center p-6 animate-in">
          <div className={`w-full max-w-sm rounded-[3rem] p-8 shadow-2xl space-y-6 ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
            ...
            <div className="flex space-x-3">
              <button onClick={() => setIsAddingExercise(false)} className="flex-1 py-4 bg-gray-100 dark:bg-gray-700 rounded-2xl font-bold text-gray-400">Cancelar</button>
              <button onClick={handleAddExercise} className="flex-1 py-4 text-white rounded-2xl font-bold" style={{ backgroundColor: accent }}>Añadir</button>
            </div>
          </div>
        </div>
      )}

      {/* Burbuja Inferior Recordatorio Peso */}
      {showReminder && (
      ...
      {showWeightForm && (
        <div className="fixed inset-0 z-[100] apple-blur flex items-center justify-center p-6 animate-in">
          <div className={`w-full max-sm rounded-[3.5rem] p-8 shadow-2xl space-y-6 ${isDark ? 'bg-gray-800' : 'bg-white'}`}>
            ...
            <div className="flex space-x-3"><button onClick={() => setShowWeightForm(false)} className="flex-1 py-4 bg-gray-100 dark:bg-gray-700 rounded-2xl font-bold text-gray-400">Cerrar</button><button onClick={handleAddWeight} className="flex-1 py-4 text-white rounded-2xl font-bold shadow-lg shadow-green-100" style={{ backgroundColor: accent }}>Guardar</button></div>
          </div>
        </div>
      )}
    </Layout>
